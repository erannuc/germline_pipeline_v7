...
def create_bash(c, outdir, sample_start, sample_end):
    ...
    sh_file = f'{outdir}/{chunk_string}_{sample_start}_{sample_end}_reduce.sh'
    with open(sh_file, 'w') as ofh:
        print('#!/bin/sh\n', file=ofh)
        print(f'/data/users/erane/germline/venv_germline/bin/python {os.getcwd()}/filter_variants_freqs_reduce_v7.py -chr {chr} -start {start} -end {end}', file=ofh)
...
def slurm_jobs(c, outdir, sample_chunks):
    ...
    for s in sample_chunks:
        if not os.path.exists(f'{outdir}/{chunk_string}_stat.vcf'):
            ...
            os.system(f'sbatch -c 2 --nice=2000 -J stat{c[0]}_{s[0]} {bash_file}')
